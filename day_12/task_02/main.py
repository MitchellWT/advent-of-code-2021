...
def visited_lower_twice(current_path: [str]) -> bool:
    lower_caves = [cave for cave in current_path if cave.islower()]
    if len(lower_caves) == len(set(lower_caves)):
        return False
    return True
    ...
    def depth_first_search(self, current_cave: str, ending_cave: str,
        ...
        if current_cave == ending_cave:
            ...
            for connected_cave in self.graph_dict[current_cave]:
                if connected_cave not in self.visited_caves or \
                        (not visited_lower_twice(current_path) and connected_cave != 'start'):
