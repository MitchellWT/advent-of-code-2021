...
if __name__ == "__main__":
    ...
    for point_list in points:
        if is_vertical_line(point_list[0], point_list[1]):
            ...
            point_one = point_list[0]
            point_two = point_list[1]
            if point_one.x < point_two.x and point_one.y < point_two.y:
                # Quadrant 1
                for i in range((point_two.x - point_one.x) + 1):
                    x_moving = point_one.x + i
                    y_moving = point_one.y + i
                    vent_diagram[y_moving][x_moving] += 1
            elif point_one.x < point_two.x and point_one.y > point_two.y:
                # Quadrant 2
                for i in range((point_two.x - point_one.x) + 1):
                    x_moving = point_one.x + i
                    y_moving = point_one.y - i
                ...
                # Quadrant 3
                for i in range((point_one.x - point_two.x) + 1):
                    x_moving = point_one.x - i
                    y_moving = point_one.y - i
                    vent_diagram[y_moving][x_moving] += 1
            elif point_one.x > point_two.x and point_one.y < point_two.y:
